...
| Layer 2 component merge | Aligned | `unlockLayer2Tower`, `_resolvePromotedTraitLoadoutForLayer`, `_createLayer2ComponentForLayer`, `Layer2ComponentState`, `AdvancementScreen` | Layer 1 tower composition creates persistent Layer 2 components. Projectile and payload affinities roll independently from completed tower mix, pure mixes reach 100%, best reached wave maps to visible Layer 2 component level, and components roll 1-3 subtraits. The inventory supports sorting, favorites, region assignment, scroll upgrades, dismantling, and region farm-output scaling. The higher-shell scaffold still exists for navigation/build space. |
