...
| Tower fabrication | Partial | `startTowerFabricationAt`, `_advanceTowerFabrication`, `OuterTowerState.isFabricating`, `tutorialStartTowerFabricationAt`, `buildTowerAt` | The player-facing tower screen now starts a timed fabrication job that reserves the slot, persists remaining time, and only counts the tower as active when complete. `buildTowerAt` remains an instant debug compatibility path, and there is no manual completion claim or trusted server timestamp yet. |
...
| Layer 2 component merge | Aligned | `unlockLayer2Tower`, `_resolvePromotedTraitLoadoutForLayer`, `_createLayer2ComponentForLayer`, `Layer2ComponentState`, `ShellPromotionPresentation`, `AdvancementScreen` | Layer 1 tower composition creates persistent Layer 2 components. Projectile and payload affinities roll independently from completed tower mix, pure mixes reach 100%, best reached wave maps to visible Layer 2 component level, and components roll 1-3 subtraits. The forecast shows key wave milestones, and the promotion result surface carries the created component so players can review, keep, and assign it. The inventory supports sorting, favorites, region assignment, scroll upgrades, dismantling, and region farm-output scaling. The higher-shell scaffold still exists for navigation/build space. |
...
| Threat Directors | Aligned | `EnemyManagerLibrary`, `_generateEnemyManager`, `assignEnemyManagerToCore`, `activeRegionThreatDirector`, `_enemyManagerEffectMultiplier`, `showThreatRegionIntelDialog` | Threat Directors use the old bible display-name roster while preserving V8-style mechanical archetypes in config summaries. They attach to Threat Map regions and tune the current spawn cadence, enemy strength, rewards, EXP, stability risk, farm wave cadence, queue disruption, Apex stability, and locked farm-wave output. Region intel now exposes this in a Layer 2 area command panel alongside push, lock, component, offline, and equipment context. |
...
1. Fabrication is only locally time-gated.
   ...
   compatibility path for debug flows. There is no trusted timestamp,
