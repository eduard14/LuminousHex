...
| Core clusters and firing queue | Aligned | `CoreState`, `AmmoPacket`, `_ammoQueue`, `_advancePulses`, `_fireCoreIfPossible`, `coreQueueCapacity` | The center core consumes relay packets as intended. One Core Manager can be assigned per shell to automate ready tower taps; managers no longer generate free core-basic packets. |
...
- Core Managers assign to one shell at a time, apply their stat board to every
  built tower in that shell, and automate ready tower taps at their automation
  rate.
